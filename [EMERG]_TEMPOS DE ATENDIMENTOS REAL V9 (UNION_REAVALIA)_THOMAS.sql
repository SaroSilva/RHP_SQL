...
, CASE WHEN (A.TEMPO_6 <= 0 OR A.TEMPO_6 IS NULL) THEN TO_CHAR(TO_DATE('00:00', 'HH24:MI') + NUMTODSINTERVAL(B.TEMPO_6, 'MINUTE'), 'HH24:MI') ELSE TO_CHAR(TO_DATE('00:00', 'HH24:MI') + NUMTODSINTERVAL(A.TEMPO_6, 'MINUTE'), 'HH24:MI') END AS HM_TEMPO6

, NVL (A.DH_PED_LAB,B.DH_PED_LAB) AS DH_PED_LAB
, NVL (A.DH_COLETA_LAB,B.DH_COLETA_LAB) AS DH_COLETA_LAB
, NVL (A.SN_LABORATORIO,B.SN_LABORATORIO) AS SN_LABORATORIO
, CASE WHEN (A.TEMPO_7 <= 0 OR A.TEMPO_7 IS NULL) THEN B.TEMPO_7 ELSE A.TEMPO_7 END AS TEMPO_7
, CASE WHEN (A.TEMPO_7 <= 0 OR A.TEMPO_7 IS NULL) THEN TO_CHAR(TO_DATE('00:00', 'HH24:MI') + NUMTODSINTERVAL(B.TEMPO_7, 'MINUTE'), 'HH24:MI') ELSE TO_CHAR(TO_DATE('00:00', 'HH24:MI') + NUMTODSINTERVAL(A.TEMPO_7, 'MINUTE'), 'HH24:MI') END AS HM_TEMPO7

, NVL (A.TIPO_EXA,B.TIPO_EXA) AS TIPO_EXA
, NVL (A.DT_PEDIDO_RX,B.DT_PEDIDO_RX) AS DT_PEDIDO_RX
, NVL (A.SN_IMAGEM,B.SN_IMAGEM) AS SN_IMAGEM
, CASE WHEN (A.TEMPO_8 <= 0 OR A.TEMPO_8 IS NULL) THEN B.TEMPO_8 ELSE A.TEMPO_8 END AS TEMPO_8
, CASE WHEN (A.TEMPO_8 <= 0 OR A.TEMPO_8 IS NULL) THEN TO_CHAR(TO_DATE('00:00', 'HH24:MI') + NUMTODSINTERVAL(B.TEMPO_8, 'MINUTE'), 'HH24:MI') ELSE TO_CHAR(TO_DATE('00:00', 'HH24:MI') + NUMTODSINTERVAL(A.TEMPO_8, 'MINUTE'), 'HH24:MI') END AS HM_TEMPO8
...
SELECT 
    ...
    END as emergencia,
        ...
        DH_COLETA_LAB,
        CASE 
          WHEN DH_PED_LAB IS NOT NULL AND HR_SOLSAI_PRO IS NOT NULL THEN 'S'
          WHEN DH_PED_LAB IS NOT NULL THEN 'S' ELSE 'N' END AS SN_LABORATORIO,
        /*  MOTIVO: VALIDAÇÃO DOS TEMPOS 
        CASE WHEN DH_PED_LAB IS NOT NULL THEN ROUND(((NVL(MAX(DH_COLETA_LAB),sysdate) - MAX (DH_PED_LAB))*24 *60),0) END AS TEMPO_7,
        CASE WHEN DH_PED_LAB IS NOT NULL THEN (ROUND(((NVL(MAX(DH_COLETA_LAB),sysdate) - MAX (DH_PED_LAB))*24 *60),0) - ROUND(((nvl(MAX(DH_DISPENSACAO_FARM),sysdate) - MAX(HR_SOLSAI_PRO))*24 *60),0)) END AS TEMPO_71,
        */
        CASE 
          ...
          ELSE ROUND((((NVL(MAX(DH_DISPENSACAO_FARM), sysdate) - MAX(HR_SOLSAI_PRO)) - (NVL(MAX(DH_COLETA_LAB), sysdate) - MAX (DH_PED_LAB))) * 24 * 60),0) END AS TEMPO_7,
        TIPO_EXA,
        DT_PEDIDO_RX,
        CASE 
          WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NOT NULL AND HR_SOLSAI_PRO IS NOT NULL THEN 'S'
          WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NULL AND HR_SOLSAI_PRO IS NOT NULL THEN 'S'
          WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NOT NULL AND HR_SOLSAI_PRO IS NULL THEN 'S'
          WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NULL AND HR_SOLSAI_PRO IS NULL THEN 'S' ELSE 'N' END AS SN_IMAGEM,
        /*  
        ROUND(((sysdate - DT_PEDIDO_RX)*24 *60),0) AS TEMPO_8,
        (ROUND(((sysdate - DT_PEDIDO_RX)*24 *60),0)-ROUND(((NVL(MAX(DH_COLETA_LAB),sysdate) - MAX (DH_PED_LAB))*24 *60),0)) AS TEMPO_81,
        */
        CASE 
          WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NULL AND HR_SOLSAI_PRO IS NULL THEN ROUND(((sysdate - DT_PEDIDO_RX)*24 *60),0)
          WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NOT NULL AND HR_SOLSAI_PRO IS NULL THEN (ROUND(((sysdate - DT_PEDIDO_RX)*24 *60),0)-ROUND(((NVL(MAX(DH_COLETA_LAB),sysdate) - MAX (DH_PED_LAB))*24 *60),0)) 
          WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NULL AND HR_SOLSAI_PRO IS NOT NULL THEN (ROUND(((sysdate - DT_PEDIDO_RX)*24 *60),0)-ROUND(((NVL(MAX(DH_DISPENSACAO_FARM),sysdate) - MAX (HR_SOLSAI_PRO))*24 *60),0))
          WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NOT NULL AND HR_SOLSAI_PRO IS NOT NULL THEN ROUND(((((sysdate - DT_PEDIDO_RX)) - ((NVL(MAX(DH_COLETA_LAB), sysdate) - MAX(DH_PED_LAB)) - (NVL(MAX(DH_DISPENSACAO_FARM), sysdate) - MAX (HR_SOLSAI_PRO)))) * 24 * 60),0) 
    --(ROUND(((sysdate - DT_PEDIDO_RX)*24 *60),0)-ROUND(((NVL(MAX(DH_COLETA_LAB),sysdate) - MAX (DH_PED_LAB))*24 *60),0)) 
        END AS TEMPO_8,
