...
SELECT 
    ...
    CASE 
      ...
      END DESC_FILA,
    CASE 
      WHEN CD_FILA_SENHA IN (19,29,182,244,246,248,161,196,210,220,221,222,245,247,335,136) THEN 'Real Vida'
      WHEN CD_FILA_SENHA IN (17,99,100,168,97,162,175,98,198) THEN 'Infante'
      WHEN CD_FILA_SENHA IN (22,60,61,170,172,178) THEN 'RM'
      WHEN CD_FILA_SENHA IN (25,26,27,43,44,45,53,54,55,56,57,58,59,164,165,171,174,228,229,230,233,234,235,278,332,343,345,346) THEN 'BV'
    END AS emergencia,
    CD_ORI_ATE,
    DS_ORI_ATE,
...
FROM (
    ...
    --FILTROS
    AND (TRUNC(DH_PROCESSO) = TRUNC(sysdate)or TRUNC(DH_PROCESSO) = TRUNC(sysdate)-1)
...
WHERE DT_ALTA_MEDICA IS NULL
AND (CD_ORI_ATE = 4 OR CD_ORI_ATE IS NULL)
AND CD_FILA_SENHA NOT IN(246, 220, 175,230,182)
--AND CD_FILA_SENHA IN (19, 29, 182, 244, 248, 161, 196, 210, 221, 222, 245, 247, 335, 136/*, 246, 220*/) -- FILAS REAL VIDA
AND NM_PACIENTE IS NOT NULL
--AND CD_ATENDIMENTO = 4660070
