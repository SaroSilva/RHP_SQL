...
TEMPO AGUARDANDO CHAMADA DO MEDICO = AT_ADM_FINAL <> BLANK() && AT_MEDICO_CHAMADA = BLANK()
TEMPO EM ATENDIMENTO MÉDICO = AT_MEDICO_CHAMADA <> BLANK() && AT_MEDICO_ALTA = BLANK()
*/
select 
...
, NVL (A.CLASS_INICIO,B.CLASS_INICIO) AS CLASS_INICIO
...
, NVL (A.AT_ADM_INICIO,B.AT_ADM_INICIO) AS AT_ADM_INICIO
...
, NVL (B.AT_MEDICO_CHAMADA,A.AT_MEDICO_CHAMADA) AS AT_MEDICO_CHAMADA
, NVL (B.AT_MEDICO_INICIO,A.AT_MEDICO_INICIO) AS AT_MEDICO_INICIO
, NVL (B.AT_MEDICO_FINAL,A.AT_MEDICO_FINAL) AS AT_MEDICO_FINAL
, NVL (B.AT_MEDICO_ALTA,A.AT_MEDICO_ALTA) AS AT_MEDICO_ALTA
...
SELECT 
    ...
    END as emergencia,
        ...
        MAX (TOTEM) AS TOTEM,
        NVL (MAX(CLASS_CHAMADA),MAX(CLASS_INICIO)) AS CLASS_CHAMADA,
        ...
        ROUND(((NVL(NVL (MAX(AT_ADM_CHAMADA), MAX(AT_ADM_INICIO)), SYSDATE)- NVL(MAX(CLASS_FINAL),SYSDATE)) * 24 * 60),0) AS TEMPO_2, --AGUARDANDO RECEPÇÃO CHAMAR
        
        NVL (MAX(AT_ADM_CHAMADA),MAX(AT_ADM_INICIO)) AS AT_ADM_CHAMADA,
        MAX(AT_ADM_INICIO) AS AT_ADM_INICIO,
        MAX(AT_ADM_FINAL) AS AT_ADM_FINAL,
        ROUND(((NVL(MAX(AT_ADM_FINAL),SYSDATE) - NVL(NVL(MAX(AT_ADM_CHAMADA),MAX(AT_ADM_INICIO)),SYSDATE))*24*60),0) AS TEMPO_3, -- ATENDIMENTO DA RECEPÇÃO
        
        ROUND((((NVL(NVL(MAX(AT_MEDICO_CHAMADA), MAX(AT_MEDICO_INICIO)),SYSDATE) - NVL (MAX(AT_ADM_FINAL),SYSDATE))) * 24 * 60),0) AS TEMPO_4, -- AGUARDANDO CHAMADA DO MEDICO
        NVL (MAX(AT_MEDICO_CHAMADA),MAX(AT_MEDICO_INICIO)) AS AT_MEDICO_CHAMADA,
        MAX(AT_MEDICO_INICIO) AS AT_MEDICO_INICIO,
        MAX(AT_MEDICO_FINAL) AS AT_MEDICO_FINAL,
        MAX(AT_MEDICO_ALTA) AS AT_MEDICO_ALTA,
        --ROUND((((SYSDATE - NVL(MAX(AT_MEDICO_INICIO),SYSDATE)))*24*60),0) AS TEMPO_5 -- EM ATENDIMENTO MEDICO
        ROUND((((NVL((SELECT MAX(PDC.DH_FECHAMENTO) 
                            ...
                            WHERE PEC.CD_DOCUMENTO_CLINICO = PDC.CD_DOCUMENTO_CLINICO
                                ...
                                ), SYSDATE) - NVL(MAX(AT_MEDICO_INICIO),SYSDATE)))*24*60),0) AS TEMPO_5, -- EM ATENDIMENTO MEDICO
                                                            
        CASE WHEN MAX(AT_MEDICO_INICIO) IS NULL THEN NULL                       
        WHEN MAX(AT_MEDICO_INICIO) IS NOT NULL AND 
        ...
        ROUND((((NVL((SELECT MAX(PDC.DH_FECHAMENTO) 
                            ...
                            WHERE PEC.CD_DOCUMENTO_CLINICO = PDC.CD_DOCUMENTO_CLINICO
                                ...
                                AND PDC.CD_ATENDIMENTO = ATENDIMENTO
                                ), SYSDATE) - NVL(MAX(AT_MEDICO_INICIO),SYSDATE)))*24*60),0) AS TEMPO_9,                                                  
        CASE 
            WHEN MAX(AT_MEDICO_INICIO) IS NULL THEN NULL                       
            WHEN MAX(AT_MEDICO_INICIO) IS NOT NULL AND                                                 
        (SELECT MAX(PDC.DH_FECHAMENTO) 
                    FROM 
                        DBAMV.PW_DOCUMENTO_CLINICO PDC, 
...
FROM (
    ...
    ,(SELECT * FROM
        (SELECT '1.LABORATORIO' TIPO,
            A.CD_ATENDIMENTO,
            MIN(DBAMV.FNC_MV_RECUPERA_DATA_HORA(P.DT_PEDIDO, P.HR_PED_LAB)) AS DH_PED_LAB,
            (SELECT MIN(DT_MOVIMENTO) AS DT_MOVIMENTO FROM DBAMV.LOG_MOVIMENTO_EXAME lme
        ...
        group by A.CD_ATENDIMENTO,P.DT_PEDIDO,P.CD_PED_LAB
) 
)LAB
            --LIGAÇÕES
