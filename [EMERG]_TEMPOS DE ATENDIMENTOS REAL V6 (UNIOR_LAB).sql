...
, DH_PEDLAB
, CASE 
    WHEN NVL (A.DH_CHECAGEM,B.DH_CHECAGEM) IS NOT NULL THEN ROUND(((nvl(NVL(MAX (DH_PEDLAB),SYSDATE) - nvl(MAX(B.DH_CHECAGEM),MAX(A.DH_CHECAGEM)),sysdate)) * 24 * 60),0) 
    WHEN NVL (A.DH_CHECAGEM,B.DH_CHECAGEM) IS NULL AND DH_PEDLAB IS NOT NULL THEN ROUND(((NVL(MAX(DH_PEDLAB),nvl(MAX(B.DH_CHECAGEM),MAX(A.DH_CHECAGEM)))-SYSDATE)* 24 * 60),0)
    ELSE 0 END AS TEMPO_7
--, DH_PEDIMA
--, CASE WHEN (A.TEMPO_6 <= 0 OR A.TEMPO_6 IS NULL) THEN B.TEMPO_6 ELSE A.TEMPO_6 END AS TEMPO_8
, A.SN_REAVALIA_MED
...
,(SELECT * FROM
        (SELECT '1.LABORATORIO' TIPO,
            ...
            TMA.CD_FILA_SENHA,
            --round((sysdate- min(DBAMV.FNC_MV_RECUPERA_DATA_HORA(p.DT_PEDIDO, p.HR_PED_LAB)))*24*60,2) DT_PEDIDO_LB,
            min(DBAMV.FNC_MV_RECUPERA_DATA_HORA(p.DT_PEDIDO, p.HR_PED_LAB)) AS DH_PEDLAB
        ...
        group by A.CD_ATENDIMENTO,PAC.NM_PACIENTE,C.NM_CONVENIO,TMA.CD_FILA_SENHA
))LAB

WHERE A.CD_ATENDIMENTO = B.CD_ATENDIMENTO(+)
AND A.CD_ATENDIMENTO = LAB.CD_ATENDIMENTO(+)
--AND A.CD_ATENDIMENTO = IMAGEM.CD_ATENDIMENTO(+)
--AND B.CD_ATENDIMENTO = LAB.CD_ATENDIMENTO(+)
--AND B.CD_ATENDIMENTO = IMAGEM.CD_ATENDIMENTO(+)
...
NVL (A.CD_ATENDIMENTO,B.CD_ATENDIMENTO)
, NVL (A.DT_ATENDIMENTO,B.DT_ATENDIMENTO)
, NVL (A.NM_PACIENTE,B.NM_PACIENTE)
, NVL (A.NM_CONVENIO,B.NM_CONVENIO)
, NVL (A.CD_FILA_SENHA,B.CD_FILA_SENHA)
, NVL (A.DS_SENHA,B.DS_SENHA)
, NVL (A.DESC_FILA,B.DESC_FILA)
, NVL (A.EMERGENCIA,B.EMERGENCIA)
, NVL (A.CD_ORI_ATE,B.CD_ORI_ATE)
, NVL (A.DS_ORI_ATE,B.DS_ORI_ATE)
, NVL (A.TOTEM,B.TOTEM)
, NVL (A.CLASS_CHAMADA,B.CLASS_CHAMADA)
, NVL (A.CLASS_INÍCIO,B.CLASS_INÍCIO)
, NVL (A.CLASS_FINAL,B.CLASS_FINAL)
, CASE WHEN (A.TEMPO_1 = 0 OR A.TEMPO_1 IS NULL) THEN B.TEMPO_1 ELSE A.TEMPO_1 END
, CASE WHEN (A.TEMPO_2 = 0 OR A.TEMPO_2 IS NULL) THEN B.TEMPO_2 ELSE A.TEMPO_2 END
, NVL (A.AT_ADM_CHAMADA,B.AT_ADM_CHAMADA)
, NVL (A.AT_ADM_INÍCIO,B.AT_ADM_INÍCIO)
, NVL (A.AT_ADM_FINAL,B.AT_ADM_FINAL)
, CASE WHEN (A.TEMPO_3 = 0 OR A.TEMPO_3 IS NULL) THEN B.TEMPO_3 ELSE A.TEMPO_3 END
, CASE WHEN (A.TEMPO_4 <= 0 OR A.TEMPO_4 IS NULL) THEN B.TEMPO_4 ELSE A.TEMPO_4 END
, NVL (B.AT_MÉDICO_CHAMADA,A.AT_MÉDICO_CHAMADA)
, NVL (B.AT_MÉDICO_INÍCIO,A.AT_MÉDICO_INÍCIO)
, NVL (B.AT_MÉDICO_FINAL,A.AT_MÉDICO_FINAL)
, NVL (B.AT_MÉDICO_ALTA,A.AT_MÉDICO_ALTA)
, CASE WHEN (A.TEMPO_5 <= 0 OR A.TEMPO_5 IS NULL) THEN B.TEMPO_5 ELSE A.TEMPO_5 END
, A.SN_ATENDIMENTO_MED
, NVL (A.DH_DISPENSACAO_FARM,B.DH_DISPENSACAO_FARM)
, NVL (A.DH_CHECAGEM,B.DH_CHECAGEM)
, CASE WHEN (A.TEMPO_6 <= 0 OR A.TEMPO_6 IS NULL) THEN B.TEMPO_6 ELSE A.TEMPO_6 END
, DH_PEDLAB
--, ROUND(((nvl(nvl(MAX(B.DH_CHECAGEM),MAX(A.DH_CHECAGEM)),sysdate) - MAX (DH_PEDLAB)) * 24 * 60),0)
--, DH_PEDIMA
--, CASE WHEN (A.TEMPO_6 <= 0 OR A.TEMPO_6 IS NULL) THEN B.TEMPO_6 ELSE A.TEMPO_6 END
, A.SN_REAVALIA_MED
, CASE WHEN (A.TEMPO_9 <= 0 OR A.TEMPO_9 IS NULL) THEN B.TEMPO_9 ELSE A.TEMPO_9 END
