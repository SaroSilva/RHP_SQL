...
FROM
    ...
    CASE
        WHEN ATE.CD_ORI_ATE = 4 THEN NVL(MAX(MES.HR_MVTO_ESTOQUE),(
        ...
        ELSE MAX(MES.HR_MVTO_ESTOQUE) END AS DH_DISPENSACAO_FARM,
    MAX(HIPC.DH_CHECAGEM) AS DH_CHECAGEM,

    CASE 
        WHEN ATE.CD_ORI_ATE = 4 THEN ROUND(((NVL(NVL(MAX(MES.HR_MVTO_ESTOQUE),(SELECT MAX(DBAMV.FNC_MV_RECUPERA_DATA_HORA(m.DT_MVTO_ESTOQUE, m.HR_MVTO_ESTOQUE)) 
                                                                                    FROM atendime a, mvto_estoque m
                                                                                    WHERE m.cd_atendimento = a.cd_atendimento
                                                                                    AND a.cd_atendimento = ATE.CD_ATENDIMENTO
                                                                                    )),SYSDATE) - MAX(SSP.HR_SOLSAI_PRO))*24 *60),0) 
        ELSE ROUND(((NVL(MAX(MES.HR_MVTO_ESTOQUE),SYSDATE)- MAX(SSP.HR_SOLSAI_PRO))*24 *60),0) END AS TEMPO_6,
