SELECT 
        ...
        ROUND(((NVL(MAX(NVL(CLASS_CHAMADA,CLASS_INICIO)),SYSDATE) - MAX(TOTEM))*24*60),2) AS TEMPO_01, --Totem até Triagem (Iniciar registro do paciente)
        
        MAX(CLASS_INICIO) AS CLASS_INICIO,
        MAX(CLASS_FINAL) AS CLASS_FINAL,
        ROUND(((NVL(MAX(CLASS_FINAL),SYSDATE) - MAX(CLASS_INICIO))*24*60),2) AS TEMPO_02, --Triagem (Classificar paciente)
        
        MAX(NVL(AT_ADM_CHAMADA,AT_ADM_INICIO)) AS AT_ADM_CHAMADA,
        MAX(AT_ADM_INICIO) AS AT_ADM_INICIO,
        MAX(AT_ADM_FINAL) AS AT_ADM_FINAL,
        ROUND(((NVL(MAX(AT_ADM_FINAL),SYSDATE) - MAX(NVL(AT_ADM_CHAMADA,AT_ADM_INICIO)))*24*60),2) AS TEMPO_03, --Recepção (Cadastro do paciente)
        
        MAX(NVL(AT_MEDICO_CHAMADA,AT_MEDICO_INICIO)) AS AT_MEDICO_CHAMADA,
        ROUND(((NVL(MAX(NVL(AT_MEDICO_CHAMADA,AT_MEDICO_INICIO)),SYSDATE) - MAX(AT_ADM_FINAL))*24*60),2) AS TEMPO_04, --Inicio do Atendimento Médico
        ...
        ROUND(((NVL((SELECT MAX(PDC.DH_FECHAMENTO) 
                            ...
                            WHERE PEC.CD_DOCUMENTO_CLINICO = PDC.CD_DOCUMENTO_CLINICO
                                ...
                                AND PEC.CD_DOCUMENTO IN (730,566,1130,1131,1298) --DOCUMENTOS DE EVOLUÇÃO MEDICA
                                AND PDC.CD_ATENDIMENTO = ATENDIMENTO
                                ), SYSDATE) - MAX(NVL(AT_MEDICO_CHAMADA,AT_MEDICO_INICIO)))*24*60),2) AS TEMPO_05, -- Em atendimento médico
                                
        MAX(HR_PRE_MED) AS DH_PRE_MED,
        MAX(HR_SOLSAI_PRO) AS DH_SOLSAI_PRO,
        MAX(DH_DISPENSACAO_FARM) AS DH_DISPENSACAO_FARM,
        ROUND(((NVL(MAX(DH_DISPENSACAO_FARM),SYSDATE) - MAX(HR_SOLSAI_PRO))*24*60),2) AS TEMPO_06, --Dispensação Farmacia
        
        MAX(DH_CHECAGEM)AS DH_CHECAGEM,
        ROUND(((NVL(MAX(DH_CHECAGEM),SYSDATE) - NVL(MAX(DH_DISPENSACAO_FARM),MAX(HR_PRE_MED)))*24*60),2) AS TEMPO_07, --Administração medicação
        ...
        (SELECT MAX(DT_MOVIMENTO) FROM DBAMV.LOG_MOVIMENTO_EXAME
         ...
         ROUND(((NVL( (SELECT MAX(DT_MOVIMENTO) FROM DBAMV.LOG_MOVIMENTO_EXAME
            ...
            FROM DBAMV.PED_LAB WHERE CD_ATENDIMENTO = ATENDIMENTO ))*24*60),2) AS TEMPO_08, --Coleta laboratorial
         
         MAX(TIPO_EXA) TIPO_EXA, 
         MAX(CD_PED_RX) CD_PED_RX, 
         MAX(DT_PEDIDO_RX) DT_PEDIDO_RX, 
         MAX(DH_REALIZADO) DH_REALIZADO,
         ROUND(((NVL(MAX(DH_REALIZADO),SYSDATE) - MAX(DT_PEDIDO_RX))*24*60),2) AS TEMPO_09, --Realização Imagem
        ...
        (SELECT MAX(PDC.DH_FECHAMENTO) 
          ...
          ROUND(((NVL(DT_ALTA_MEDICA,SYSDATE) - (SELECT MAX(PDC.DH_FECHAMENTO) 
                                                  FROM 
                                                    ...
                                                    WHERE PEC.CD_DOCUMENTO_CLINICO = PDC.CD_DOCUMENTO_CLINICO
                                                      ...
                                                      AND PEC.CD_DOCUMENTO IN (731,1132) ----FICHA DE REAVALIZAÇÃO
                                                      AND PDC.CD_ATENDIMENTO = ATENDIMENTO
                                                  ))*24*60),2) AS TEMPO_10, --Realização de reavaliação
          ...
          CASE 
            WHEN (SELECT MAX(DH_FECHAMENTO) FROM DBAMV.PW_DOCUMENTO_CLINICO WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12) IS NOT NULL THEN 0
            ELSE ROUND(((NVL(DT_ALTA_MEDICA,SYSDATE) - (SELECT MAX(DH_FECHAMENTO) FROM DBAMV.PW_DOCUMENTO_CLINICO
                                                        WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12))*24*60),2) END AS TEMPO_11 --Aguardadndo Internamento
...
FROM (
            SELECT 
                ...
                (SELECT DS_TIPO_RISCO FROM DBAMV.SACR_CLASSIFICACAO WHERE CD_CLASSIFICACAO = TMA.CD_CLASSIFICACAO) AS TP_CLASSIFICACAO,
                DECODE (TMA.TP_SEXO,'M','MASCULINO','F','FEMININO') AS TP_SEXO,
                ATE.CD_CID,
                (SELECT DS_CID FROM DBAMV.CID WHERE CD_CID = ATE.CD_CID) AS DS_CID,
                TMP_IMG.TIPO_EXA, 
                TMP_IMG.CD_PED_RX, 
                TMP_IMG.DT_PEDIDO_RX, 
                TMP_IMG.DH_REALIZADO
    ...
    ,(
      ...
      WHERE NR_ORD = 1
            ...
            AND ATE.CD_ATENDIMENTO = TMP_IMG.CD_ATENDIMENTO(+)
            ...
            AND ATE.CD_ATENDIMENTO = 4761013
