SELECT DISTINCT
      ...
      (NVL(A.DT_FINAL_CONTRATO, B.DT_FINAL_CONTRATO)-TRUNC(SYSDATE)) AS SLA,
      --CASE WHEN (COALESCE(A.DT_FINAL_CONTRATO, B.DT_FINAL_CONTRATO)-TRUNC(SYSDATE))>0 THEN (COALESCE(A.DT_FINAL_CONTRATO, B.DT_FINAL_CONTRATO)-TRUNC(SYSDATE)) ELSE (COALESCE(A.DT_FINAL_CONTRATO, B.DT_FINAL_CONTRATO)-TRUNC(SYSDATE))*-1 END AS SLA,
      A.DT_ELAB_CONTRATO,
...
SELECT
    ...
    REPLACE(AHV.NR_INDICE_HCT, '%', '') AS NR_INDICE_HCT, -- ADC INDICE DE SEGURANÇA Honorários médicos || Consultas || Terapias UTILIZADO PARA MEDIDA
    REPLACE(AHV.NR_INDICE_DTP, '%', '') AS NR_INDICE_DTP, -- ADC INDICE DE SEGURANÇA Diarias || Taxas || Pacotes UTILIZADO PARA MEDIDA
    REPLACE(AHV.NR_INDICE_SAD, '%', '') AS NR_INDICE_SAD, -- ADC INDICE DE SEGURANÇA Serviços Auxiliares de Diagnósticos. UTILIZADO PARA MEDIDA
    REPLACE(AHV.NR_INDICE_CONTRATUAL, '%', '') AS NR_INDICE_CONTRATUAL,
    CASE WHEN AHV.TP_REAJUSTE = 'true' THEN 'Ativado' ELSE 'Inativado' END AS TP_REAJUSTE, --ADC PARA ATIVAÇÃO DE MEDIDA DOS VALORES DE SEGURANÇA
    AHV.DS_OBS_CADASTRO,
