...
,CASE
    WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NOT NULL AND HR_SOLSAI_PRO IS NOT NULL THEN ROUND(((SYSDATE - DT_PEDIDO_RX)*24 *60),0) - (ROUND(((NVL(MAX(DH_COLETA_LAB), sysdate) - MAX(DH_PED_LAB))* 24 * 60),0) + ROUND(((NVL(MAX(DH_DISPENSACAO_FARM), sysdate) - MAX(HR_SOLSAI_PRO)) * 24 * 60),0) )
    WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NULL AND HR_SOLSAI_PRO IS NOT NULL THEN ROUND(((SYSDATE - DT_PEDIDO_RX)*24 *60),0) - (ROUND(((NVL(MAX(DH_DISPENSACAO_FARM), sysdate) - MAX(HR_SOLSAI_PRO)) * 24 * 60),0)*-1)
    ...
    WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NULL AND HR_SOLSAI_PRO IS NULL THEN ROUND(((SYSDATE - DT_PEDIDO_RX)*24 *60),0) END AS TEMPO_88
, TO_CHAR(TO_DATE('00:00', 'HH24:MI') + NUMTODSINTERVAL(
CASE
    WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NOT NULL AND HR_SOLSAI_PRO IS NOT NULL THEN ROUND(((SYSDATE - DT_PEDIDO_RX)*24 *60),0) - (ROUND(((NVL(MAX(DH_COLETA_LAB), sysdate) - MAX(DH_PED_LAB))* 24 * 60),0) + ROUND(((NVL(MAX(DH_DISPENSACAO_FARM), sysdate) - MAX(HR_SOLSAI_PRO)) * 24 * 60),0) )
    WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NULL AND HR_SOLSAI_PRO IS NOT NULL THEN ROUND(((SYSDATE - DT_PEDIDO_RX)*24 *60),0) - (ROUND(((NVL(MAX(DH_DISPENSACAO_FARM), sysdate) - MAX(HR_SOLSAI_PRO)) * 24 * 60),0)*-1)
    WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NOT NULL AND HR_SOLSAI_PRO IS NULL THEN ROUND(((SYSDATE - DT_PEDIDO_RX)*24 *60),0) - ROUND(((NVL(MAX(DH_COLETA_LAB), sysdate) - MAX(DH_PED_LAB))* 24 * 60),0)
    WHEN DT_PEDIDO_RX IS NOT NULL AND DH_PED_LAB IS NULL AND HR_SOLSAI_PRO IS NULL THEN ROUND(((SYSDATE - DT_PEDIDO_RX)*24 *60),0) END
, 'MINUTE'), 'HH24:MI') AS HM_TEMPO88
FROM (
    SELECT 
...
;
SELECT CD_SET_EXA, NM_SET_EXA, SE.TP_SETOR, ST.NM_SETOR FROM SET_EXA SE, SETOR ST
WHERE ST.CD_SETOR = SE.CD_SETOR
AND NM_SETOR LIKE '%IMA%'
ORDER BY 1