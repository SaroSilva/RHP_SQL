SELECT 
        ...
        CASE
          ...
          ELSE ROUND(((NVL(MAX(NVL(CLASS_CHAMADA,CLASS_INICIO)),SYSDATE) - MAX(TOTEM))*24*60),0) 
        ...
        ROUND(((NVL(NVL((SELECT MAX(PDC.DH_FECHAMENTO) 
                            ...
                            WHERE PEC.CD_DOCUMENTO_CLINICO = PDC.CD_DOCUMENTO_CLINICO
                                ...
                                AND PEC.CD_DOCUMENTO IN (730,566,1130,1131,1298) --DOCUMENTOS DE EVOLUÇÃO MEDICA
                                AND PDC.CD_ATENDIMENTO = ATENDIMENTO
                                ),MAX(AT_MEDICO_FINAL)), SYSDATE) - MAX(NVL(AT_MEDICO_CHAMADA,AT_MEDICO_INICIO)))*24*60),0) AS TEMPO_05, -- Em atendimento médico
        ...
        CASE
          ...
          ELSE ROUND(((NVL(MAX(DH_CHECAGEM),SYSDATE) - NVL(MAX(DH_DISPENSACAO_FARM),MAX(HR_PRE_MED)))*24*60),0) END AS TEMPO_07, --Administração medicação
        ...
        (SELECT MAX(PDC.DH_FECHAMENTO) 
          ...
          ROUND(((NVL((SELECT MAX(PDC.DH_FECHAMENTO) 
                        ...
                        WHERE PEC.CD_DOCUMENTO_CLINICO = PDC.CD_DOCUMENTO_CLINICO
                            ...
                            AND PEC.CD_DOCUMENTO IN (731,1132) ----FICHA DE REAVALIZAÇÃO
                            AND PDC.CD_ATENDIMENTO = ATENDIMENTO
                        ),SYSDATE)-(SELECT MAX(PDC.DH_FECHAMENTO) 
                                      FROM DBAMV.PW_DOCUMENTO_CLINICO PDC
                                      WHERE PDC.CD_OBJETO = 412
                                      AND PDC.CD_ATENDIMENTO = ATENDIMENTO))*24*60),0) AS TEMPO_10,
