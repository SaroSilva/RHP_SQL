...
,EXTRACT(HOUR FROM (NVL(AHT.END_TIME_,SYSDATE)) - AHT.START_TIME_) || ':' || EXTRACT(MINUTE FROM (NVL(AHT.END_TIME_,SYSDATE)) - AHT.START_TIME_) || ':' || TRUNC(EXTRACT(SECOND FROM (NVL(AHT.END_TIME_,SYSDATE)) - AHT.START_TIME_)) AS TMP_SLA
,ROUND(SUBSTR((((NVL(AHT.END_TIME_,SYSDATE)) - AHT.START_TIME_)*24*60), 2, INSTR((((NVL(AHT.END_TIME_,SYSDATE)) - AHT.START_TIME_)*24*60), ' ')- 2),0) AS TMP_MIN
