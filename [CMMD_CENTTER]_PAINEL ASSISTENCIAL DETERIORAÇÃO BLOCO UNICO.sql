SELECT OWNER, TABLE_NAME FROM SYS.ALL_TAB_COLUMNS
WHERE OWNER = 'DBAMV'
AND TABLE_NAME LIKE '%PW_%PROTOCOLO%'
AND COLUMN_NAME = 'CD_CASO_PROTOCOLO'
ORDER BY 2
;
...
,ROUND (((SYSDATE - SCOR.DH_FECHAMENTO) * 1440),2)AS TMP_ULTIMO_SCORE
,ENTRADA.DH_FECHAMENTO AS ENTRADA_DETERIORACAO --Verificado o ultimo documento 1921 que foi preenchido para o paciente
,CASE WHEN ENTRADA.DH_FECHAMENTO IS NULL THEN ROUND (((SYSDATE - NVL (ENTRADA.DH_FECHAMENTO,SCOR.DH_FECHAMENTO)) * 1440),2) ELSE 0 END AS TMP_ENTRADA_DETERIORACAO
