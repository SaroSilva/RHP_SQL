SELECT 
        ...
        CASE
          ...
          ELSE ROUND(((NVL(NVL(MAX(NVL(CLASS_CHAMADA,CLASS_INICIO)),NVL((SELECT MAX(DH_FECHAMENTO) FROM DBAMV.PW_DOCUMENTO_CLINICO
                                                                          WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12),DT_ALTA_MEDICA)),SYSDATE) - MAX(TOTEM))*24*60),0) 
        ...
        MAX(NVL(AT_MEDICO_INICIO,AT_MEDICO_CHAMADA)) AS AT_MEDICO_CHAMADA,
        CASE 
          WHEN MAX(NVL(AT_MEDICO_INICIO,AT_MEDICO_CHAMADA)) IS NULL AND DT_ALTA_MEDICA IS NOT NULL THEN ROUND(((NVL(NVL((SELECT MAX(DH_FECHAMENTO) FROM DBAMV.PW_DOCUMENTO_CLINICO
                                                                                                                          WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12),DT_ALTA_MEDICA),SYSDATE) - MAX(AT_ADM_FINAL))*24*60),0)
          ELSE ROUND(((NVL(MAX(NVL(AT_MEDICO_INICIO,AT_MEDICO_CHAMADA)),SYSDATE) - MAX(AT_ADM_FINAL))*24*60),0) END AS TEMPO_04, --Inicio do Atendimento Médico
        ...
        ROUND(((NVL(NVL((SELECT MAX(PDC.DH_FECHAMENTO) 
                            ...
                            WHERE PEC.CD_DOCUMENTO_CLINICO = PDC.CD_DOCUMENTO_CLINICO
                                ...
                                AND PEC.CD_DOCUMENTO IN (730,566,1130,1131,1298) --DOCUMENTOS DE EVOLUÇÃO MEDICA
                                AND PDC.CD_ATENDIMENTO = ATENDIMENTO
                                ),NVL((SELECT MAX(DH_FECHAMENTO) FROM DBAMV.PW_DOCUMENTO_CLINICO
                                       WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12),DT_ALTA_MEDICA)), SYSDATE) - MAX(NVL(AT_MEDICO_INICIO,AT_MEDICO_CHAMADA)))*24*60),0) >= 0 --AT_MEDICO_INICIO
        THEN
        ROUND(((NVL(NVL((SELECT MAX(PDC.DH_FECHAMENTO) 
                            FROM 
                                DBAMV.PW_DOCUMENTO_CLINICO PDC, 
                            ...
                            WHERE PEC.CD_DOCUMENTO_CLINICO = PDC.CD_DOCUMENTO_CLINICO
                                ...
                                AND PDC.CD_ATENDIMENTO = ATENDIMENTO
                                       ...
                                       WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12),DT_ALTA_MEDICA)), SYSDATE) - MAX(NVL(AT_MEDICO_INICIO,AT_MEDICO_CHAMADA)))*24*60),0) 
        ELSE 
        ROUND(((NVL(NVL((SELECT MAX(PDC.DH_FECHAMENTO) 
                            FROM 
                                DBAMV.PW_DOCUMENTO_CLINICO PDC, 
                            ...
                            WHERE PEC.CD_DOCUMENTO_CLINICO = PDC.CD_DOCUMENTO_CLINICO
                                ...
                                AND PDC.CD_ATENDIMENTO = ATENDIMENTO
                                        ...
                                        WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12),DT_ALTA_MEDICA)), SYSDATE) - MAX(NVL(AT_MEDICO_INICIO,AT_MEDICO_CHAMADA)))*24*60),0) * -1
        ...
        CASE
          WHEN MAX(HR_SOLSAI_PRO) IS NOT NULL AND MAX(DH_DISPENSACAO_FARM) IS NULL AND DT_ALTA_MEDICA IS NOT NULL THEN ROUND(((NVL(NVL((SELECT MAX(DH_FECHAMENTO) FROM DBAMV.PW_DOCUMENTO_CLINICO
                                                                                                                                        WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12),DT_ALTA_MEDICA),SYSDATE) - MAX(HR_SOLSAI_PRO))*24*60),0)
        ...
        CASE
          WHEN MAX(HR_SOLSAI_PRO) IS NOT NULL AND MAX(DH_DISPENSACAO_FARM) IS NULL AND DT_ALTA_MEDICA IS NOT NULL AND MAX(DH_CHECAGEM) IS NULL THEN ROUND(((NVL(NVL((SELECT MAX(DH_FECHAMENTO) FROM DBAMV.PW_DOCUMENTO_CLINICO
                                                                                                                                                                      WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12),DT_ALTA_MEDICA),SYSDATE) - NVL(MAX(DH_DISPENSACAO_FARM),MAX(HR_PRE_MED)))*24*60),0)
          ELSE ROUND(((NVL(NVL(MAX(DH_CHECAGEM),NVL((SELECT MAX(DH_FECHAMENTO) FROM DBAMV.PW_DOCUMENTO_CLINICO
                                                      WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12),DT_ALTA_MEDICA)),SYSDATE) - NVL(MAX(DH_DISPENSACAO_FARM),MAX(HR_PRE_MED)))*24*60),0) END AS TEMPO_07, --Administração medicação
        ...
        (SELECT MAX(PDC.DH_FECHAMENTO) 
          ...
          ROUND(((NVL(NVL((SELECT MAX(PDC.DH_FECHAMENTO) 
                        ...
                        WHERE PEC.CD_DOCUMENTO_CLINICO = PDC.CD_DOCUMENTO_CLINICO
                            ...
                            AND PDC.CD_ATENDIMENTO = ATENDIMENTO
                                ...
                                WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12),DT_ALTA_MEDICA)),SYSDATE)-(SELECT MAX(PDC.DH_FECHAMENTO) 
                                                                                                                            FROM DBAMV.PW_DOCUMENTO_CLINICO PDC
                                                                                                                            WHERE PDC.CD_OBJETO = 412
          ...
          ROUND(((NVL(NVL((SELECT MAX(DH_FECHAMENTO) FROM DBAMV.PW_DOCUMENTO_CLINICO
                            WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12),DT_ALTA_MEDICA),SYSDATE) - (SELECT MAX(PDC.DH_FECHAMENTO) 
                                                  FROM 
                                                      DBAMV.PW_DOCUMENTO_CLINICO PDC, 
                                                      DBAMV.PW_EDITOR_CLINICO PEC,
         ...
         CASE 
          ...
          ROUND(((NVL(NVL((SELECT MAX(DH_FECHAMENTO) FROM DBAMV.PW_DOCUMENTO_CLINICO
                                WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12),DT_ALTA_MEDICA),SYSDATE) - (SELECT MAX(DH_FECHAMENTO) FROM DBAMV.PW_DOCUMENTO_CLINICO
                                                                                                                              WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12))*24*60),0) AS TEMPO_12,
         
          ROUND(((NVL(NVL((SELECT MAX(DH_FECHAMENTO) FROM DBAMV.PW_DOCUMENTO_CLINICO
                                WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12),DT_ALTA_MEDICA),CASE 
                    WHEN (DT_ATENDIMENTO + INTERVAL '96' HOUR ) < SYSDATE 
                    THEN (TRUNC (DT_ATENDIMENTO) + INTERVAL '96' HOUR)
                    ELSE SYSDATE
                END) -  (NVL(NVL(NVL(MAX(TOTEM),MAX(AT_ADM_INICIO)),MAX(AT_ADM_FINAL)),(SELECT MAX(PDC.DH_FECHAMENTO) 
                                                                                                                                                                                              FROM 
                                                                                                                                                                                                  DBAMV.PW_DOCUMENTO_CLINICO PDC, 
                                                                                                                                                                                                  DBAMV.PW_EDITOR_CLINICO PEC,
                                                                                                                                                                                                  DBAMV.PRESTADOR PRD, 
          ...
          TO_CHAR(TO_DATE('00:00', 'HH24:MI') + NUMTODSINTERVAL(ROUND(((NVL(NVL((SELECT MAX(DH_FECHAMENTO) FROM DBAMV.PW_DOCUMENTO_CLINICO
                                                                                  WHERE CD_ATENDIMENTO = ATENDIMENTO AND CD_TIPO_DOCUMENTO = 12),DT_ALTA_MEDICA),CASE 
                    WHEN (DT_ATENDIMENTO + INTERVAL '48' HOUR ) < SYSDATE 
                    THEN (TRUNC (DT_ATENDIMENTO) + INTERVAL '48' HOUR)
                    ELSE SYSDATE
                END) -  
                                                                                  
                                                                                  (NVL(NVL(NVL(MAX(TOTEM),MAX(AT_ADM_INICIO)),MAX(AT_ADM_FINAL)),(SELECT MAX(PDC.DH_FECHAMENTO) 
                                                                                                                                                    FROM 
                                                                                                                                                        DBAMV.PW_DOCUMENTO_CLINICO PDC, 
                                                                                                                                                        DBAMV.PW_EDITOR_CLINICO PEC,
                                                                                                                                                        DBAMV.PRESTADOR PRD, 
                                                                                                                                                    ...
                                                                                                                                                    WHERE PEC.CD_DOCUMENTO_CLINICO = PDC.CD_DOCUMENTO_CLINICO
                                                                                                                                                        ...
                                                                                                                                                        AND PEC.CD_DOCUMENTO IN (730,566,1130,1131,1298) --DOCUMENTOS DE EVOLUÇÃO MEDICA
                                                                                                                                                        AND PDC.CD_ATENDIMENTO = ATENDIMENTO
                                                                                                                                                        ))))*24*60),0), 'MINUTE'), 'HH24:MI') AS HM_TEMPO20
...
FROM (
            ...
            FROM 
                ...
                WHERE
                    ...
                    --AND ATE.CD_ATENDIMENTO = 4543973
                GROUP BY
                    ATE.CD_ATENDIMENTO,
                    ATE.CD_ORI_ATE,
                    ATE.CD_CONVENIO,
        ...
        WHERE
            ...
            AND ATE.DT_ATENDIMENTO >= '01/JAN/2024'
            AND ATE.DT_ATENDIMENTO < TRUNC(SYSDATE)+INTERVAL '24' HOUR
            --AND ATE.CD_ATENDIMENTO IN (4543973)
